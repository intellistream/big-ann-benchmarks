...
class test_HNSW(BaseStreamingANN):
    ...
    def setup(self, dtype, max_pts, ndim):
        ...
        # Mapping from test index ids to global id
        # suppose it initial loads a data [5000:10000], test index would be [0:5000] my_index[0:5000] = [5000:10000]
        self.my_index = -1 * np.ones(max_pts, dtype=int)
        # Mapping from global id to test index id
    ...
    def insert(self, X,ids):
        ...
        if(new_data.shape[0]!=0):
            ...
            print(f"test indices {indices[0]} : {indices[-1]} to Global {new_ids[0]}:{new_ids[-1]}")
    ...
    def index_name(self, name):
        return f"data/{name}.{self.indexkey}.testindex"