...
def plot_recall_vs_batch_rate(file_path, save_path):
    ...
    # Filter rows with dataset containing "batch" between 1000-50000
    data = data[data['dataset'].str.contains(r'randomDrop(0.05|0.10|0.15|0.20|0.25)\.yaml')]

    # Filter rows for specific algorithms
    data = data[data['algorithm'].isin(algorithms)]
    data["randomDrop"] = data["dataset"].str.extract(r"randomDrop(\d+\.\d{2})\.yaml").astype(float)
...
def plot_batch_rate_congestion(file_path, save_path):
    ...
    unique_values = sorted(data["randomDrop"].unique())
    ...
    for (algorithm, group), (marker, color) in zip(data.groupby("algorithm"), zip(markers, colors)):
        group = group.sort_values(by="randomDrop")
        plot_lines.append([])
        for value, sub_group in group.groupby("randomDrop"):
            l1 = plt.scatter(
                ...
                s=150,
                label=algorithm if value == group["randomDrop"].iloc[0] else "",
