...
def plot_recall_vs_batch_rate(file_path, save_path):
    ...
    # Filter rows with dataset containing "batch" between 1000-50000
    data = data[data['dataset'].str.contains(r'wte-(0\.05|0\.2|0\.4|0\.6|0\.8)')]

    # Filter rows for specific algorithms
    data = data[data['algorithm'].isin(algorithms)]
    data["wordContamination"] = data["dataset"].str.extract(r"wte-(0\.05|0\.2|0\.4|0\.6|0\.8)").astype(float)

    plt.figure(figsize=(7, 6))
    for (algorithm, group), (marker, color) in zip(data.groupby("algorithm"), zip(markers, colors)):
        group = group.sort_values(by="wordContamination")
        x = group["wordContamination"].to_numpy()
    ...
    plt.xlabel("Hot Spot Proportion",fontsize=TICK_FONT_SIZE)
    plt.ylabel("Recall@10",fontsize=TICK_FONT_SIZE)
    plt.xticks([0.05, 0.2, 0.4, 0.6, 0.8])
...
def plot_batch_rate_congestion(file_path, save_path):
    ...
    unique_values = sorted(data["wordContamination"].unique())
    ...
    for (algorithm, group), (marker, color) in zip(data.groupby("algorithm"), zip(markers, colors)):
        group = group.sort_values(by="wordContamination")
        plot_lines.append([])
        for value, sub_group in group.groupby("wordContamination"):
            l1 = plt.scatter(
                ...
                s=150,
                label=algorithm if value == group["wordContamination"].iloc[0] else "",
...
# File paths
input_file = "F7_wordContamination.csv"
output_dir = "scripts/plotting/plots/wordContamination/"
