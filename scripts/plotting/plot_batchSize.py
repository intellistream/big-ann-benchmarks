...
def plot_batch_rate_congestion(file_path, save_path):
    ...
    # Sort unique Batch Sizes for consistent marker assignment
    unique_values = sorted(data["batchSize"].unique())
    ...
    for (algorithm, group), (marker, color) in zip(data.groupby("algorithm"), zip(markers, colors)):
        group = group.sort_values(by="batchSize")
        plot_lines.append([])
        for value, sub_group in group.groupby("batchSize"):
            l1 = plt.scatter(
                ...
                s=150,
                label=algorithm if value == group["batchSize"].iloc[0] else "",
